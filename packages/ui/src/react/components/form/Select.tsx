...
const variantStyles: Record<SelectVariant, string> = {
  ...
  dark:  "border-white/10 bg-white/5 text-white/85 focus:ring-white/20",
};
    ...
    <div className={cn("space-y-1 relative", className)} ref={ref}>
      ...
      {open && (
        <div className={cn(
          ...
          isDark ? "border-white/15 bg-[#0f1a2e]" : "border-slate-200 bg-white"
        )}>
          <div className={cn("p-2 border-b", isDark ? "border-white/10" : "border-slate-100")}>
            <input
              ...
              className={cn(
                ...
                isDark ? "border-white/10 bg-white/5 text-white/85 placeholder:text-white/30"
          ...
          <ul className="max-h-52 overflow-auto py-1">
            {filtered.length === 0 && (
              <li className={cn("px-3 py-2 text-sm", isDark ? "text-white/40" : "text-slate-400")}>Sin resultados</li>
            )}
            {filtered.map((opt) => {
                ...
                <li key={opt.value}>
                  <button type="button" disabled={opt.disabled} onClick={() => toggle(opt.value)}
                    className={cn(
                      ...
                      isDark
                        ? sel ? "bg-white/10 text-white" : "text-white/70 hover:bg-white/5"
                    ...
                    <span className={cn("w-4 h-4 rounded border flex items-center justify-center shrink-0",
                      sel
                        ? isDark ? "bg-white/90 border-white/90" : "bg-slate-900 border-slate-900"
          ...
          {value.length > 0 && (
            <div className={cn("px-3 py-2 border-t", isDark ? "border-white/10" : "border-slate-100")}>
              <button type="button" onClick={() => onChange([])}
                className={cn("text-xs transition", isDark ? "text-white/40 hover:text-white/70" : "text-slate-500 hover:text-slate-800")}>
                Limpiar selección
              </button>
            </div>
          )}
              ...
              <li key={opt.value}>
                <button type="button" disabled={opt.disabled}
                  ...
                  className={cn(
                    "w-full flex items-center gap-2 px-3 py-2 text-sm text-left transition",
                    isDark
                      ? value === opt.value ? "bg-white/10 text-white font-medium" : "text-white/70 hover:bg-white/5"
