...
const EMPTY_FORM = {
  ...
  tiempo_horas: "0",
  tiempo_minutos: "0",
  tamano_base_mm: "",   // Ancho (X) — dimensión dominante para precios
  tamano_y_mm: "",      // Alto  (Y)
  tamano_z_mm: "",      // Largo (Z)
  tamano_minimo_mm: "",
  tamano_maximo_mm: "",
...
export default function CatalogoProductoDetail() {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const { tiempo_horas, tiempo_minutos, ...rest } = form;
      const payload = {
        ...rest,
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl p-6 space-y-6" style={{ background: "var(--c-shell)", border: "1px solid var(--c-border-med)" }}>
        ...
        {/* Tiempo */}
        <Field label="Tiempo de impresión base *" hint="medido en las dimensiones base">
        ...
        <div>
          <p className="text-xs font-medium mb-2" style={{ color: "var(--c-text-4)" }}>
            Dimensiones base (mm) <span className="font-normal opacity-70"> — medidas reales de la pieza a tamaño base</span>
          </p>
          <div className="grid grid-cols-3 gap-3">
            <Field label="Ancho (X) *" hint="dimensión principal">
              <Input type="number" value={form.tamano_base_mm} onChange={f("tamano_base_mm")} placeholder="120" />
            </Field>
            <Field label="Alto (Y)" hint="opcional">
              <Input type="number" value={form.tamano_y_mm} onChange={f("tamano_y_mm")} placeholder="90" />
            </Field>
            <Field label="Largo (Z)" hint="opcional">
              <Input type="number" value={form.tamano_z_mm} onChange={f("tamano_z_mm")} placeholder="80" />
            </Field>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Tamaño mínimo (mm) *" hint="límite inferior del slider">
            <Input type="number" value={form.tamano_minimo_mm} onChange={f("tamano_minimo_mm")} placeholder="20" />
          </Field>
          <Field label="Tamaño máximo (mm) *" hint="límite superior del slider">
